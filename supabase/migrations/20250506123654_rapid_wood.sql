/*
  # Fix AI Coach Edge Function

  1. Changes
    - Fix duplicate code in the ai-coach edge function
...
-- Create a function to handle development plan generation
CREATE OR REPLACE FUNCTION generate_development_plan_from_assessment(assessment_id uuid)
RETURNS jsonb AS $$
DECLARE
...
-- Create a function to handle fallback plan generation when OpenAI is unavailable
CREATE OR REPLACE FUNCTION create_fallback_development_plan()
RETURNS TRIGGER AS $$
BEGIN
  -- Generate development plan using the database function
  INSERT INTO development_plans (
    assessment_id,
    plan_data
  ) VALUES (
    NEW.id,
    generate_development_plan_from_assessment(NEW.id)
  )
  ON CONFLICT (assessment_id) 
  DO UPDATE SET
    plan_data = generate_development_plan_from_assessment(NEW.id),
    updated_at = now();
...
-- Create trigger to automatically generate fallback development plan on assessment completion
CREATE TRIGGER create_fallback_development_plan_trigger
  AFTER INSERT ON assessment_history
  FOR EACH ROW
  EXECUTE FUNCTION create_fallback_development_plan();